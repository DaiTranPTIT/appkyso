...
import { cmcSign, requestSignCMC } from '@/services/base/api';
import { ICMCSign, ICMCSignInfo } from '@/services/base/typing';
...
export default () => {
		...
		if (dsKy[0] && initialLocation && pdfLoaded) {
			setTimeout(() => {
				...
				setSignaturePosition({
					x: (containerWidth * initialLocation.x) / 100,
					y:
						containerHeight -
						(initialLocation.height * containerHeight) / 100 -
						(containerHeight * initialLocation.y) / 100,
				});
	...
	const cmcRequestSigning = async () => {
		try {
			...
			setLoadingBtn(true);
			const request: ICMCSignInfo = {
				sign_info_id: id,
				message: 'Tôi đồng ý ký số',
				page_width: String(container_width),
				page_height: String(container_height),
	...
	const showModalInputOtp = async (data: any) => {
		Modal.confirm({
			title: 'Nhập mã OTP xác thực',
			onOk: () => {
				return new Promise((resolve, reject) => {
					form
						.validateFields()
						.then(async (values) => {
							const params: ICMCSign = {
								sign_info_id: id,
								session_id: data.detail.sessionId,
								otp: values.otp,
							};
							try {
								setLoadingKy(true);
			...
			content: (
				<Form form={form} layout='vertical'>
					<Form.Item name='otp' rules={[...rules.required]} label='Mã OTP'>
	...
	const layToaDo = async (acc: { username: string; password: string }) => {
														...
														columns={[
															{
																...
																render: (_: any, record: any) => <Radio value={record.credential_id} />,
															},
															{
																title: 'Cred',
																dataIndex: 'credName',
		...
		<>
			{!pdfLoaded ? (
				...
				<div style={{ background: '#f4f4f4' }}>
					...
					<div className='flex justify-between'>
									...
									<Row gutter={[10, 10]}>
										{dsKy?.map((item) => (
											<Col
												...
												className={`bg-white border-gray-500 p-2 rounded shadow-md chu-ky ${
													item === chuKyDrop?.chuKy && 'active'
												}`}
											>
												<div
												...
												>
													<img
														src={`${ipServiceKy}${item.file_path}`}
														style={{ height: '40px', objectFit: 'contain', background: 'white' }}
														alt='Signature'
													/>
											...
											return (
												<li key={index}>
													<Tag
														...
														color={item === signatureArea ? 'red' : undefined}
													>
														Trang {item.page}
													</Tag>
												</li>
						...
						<div className='lg:w-[calc(100%-400px)] w-[calc(100%-300px)] margin-[auto] h-[100vh] overflow-auto'>
							...
							<div className='py-4'>
								{(pdfFile && (
									<div
										className={`relative border-2 border-dashed border-${
											isDragging ? 'blue-500' : 'grey'
										} p-2 container-drag w-[max-content] bg-${isDragging ? 'blue-50' : ''} mx-[auto]`}
									>
										{
										...
										{signaturePosition && (
											...
											>
												...
												>
													<div className='absolute close-button' style={{ zIndex: 1 }} onClick={removeChuKy}>
														<CloseOutlined style={{ fontSize: '8px' }} />
													</div>
													<ResizableBox
														width={
															initialLocation && pdfContainerRef.current?.getBoundingClientRect()
																? (initialLocation?.width * pdfContainerRef.current?.getBoundingClientRect().width) /
																  100
																: 100
														}
														height={
															initialLocation && pdfContainerRef.current?.getBoundingClientRect()
																? (initialLocation?.height * pdfContainerRef.current?.getBoundingClientRect().height) /
																  100
																: 100
														}
													...
													>
														<img
															...
															src={`${ipServiceKy}${chuKyDrop?.chuKy?.file_path}`}
															alt='Signature'
														/>
													</ResizableBox>
												</div>
								...
								)) || <Spin className='flex items-center w-[100%]' />}
							</div>
						</div>
					</div>
					...
					<Modal title='Ký số' visible={loadingKy} footer={false} onCancel={() => setLoadingKy(false)}>
						{loadingKy && (
							<div className='signing'>
								<img src='/kyso/signing.gif' width={150} />
								<strong>Đang trong quá trình ký số ...</strong>
							</div>
						)}
					</Modal>
				</div>
